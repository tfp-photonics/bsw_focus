...
info = '/Users/yannick/Desktop/rebuttal/results/feat_depth.txt'
names = [
    '/Users/yannick/Desktop/rebuttal/foc_depth/20180726-102632_bsw_20x5_res5_fields.h5',
    '/Users/yannick/Desktop/rebuttal/foc_depth/20180726-105802_bsw_40x10_res5_fields.h5',
    '/Users/yannick/Desktop/rebuttal/foc_depth/20180726-125221_bsw_60x15_res5_fields.h5',
    '/Users/yannick/Desktop/rebuttal/foc_depth/20180726-161735_bsw_80x20_res5_fields.h5',
]
...
feat = np.ma.array(np.array([2.0, 1.6667, 1.4286, 1.25, 1.1111, 1.0, 0.9091, 0.8333,
                    0.7692, 0.7143, 0.6667, 0.5882, 0.5]) / lbsw, mask=False)
fwhm = np.ma.array(data[:, 0], mask=False)
depth = np.ma.array(data[:, 1], mask=False)

ax5.plot(feat, depth, 'g--', lw=0.7, zorder=1)
ax5.set_ylim([0, 7.5])
ax5.set_yticks([0, 1, 2, 3, 4, 5, 6, 7])
ax5.set_yticklabels([str(x) for x in [0, 1, 2, 3, 4, 5, 6, 7]], fontsize=8)
ax5.set_ylabel(r'Axial FWHM ($\lambda_{\text{BSW}}$)', color='g', fontsize=8)

ax6 = ax5.twinx()
ax6.plot(feat, fwhm, 'r--', lw=0.7, zorder=1)
ax6.set_ylim([0.0, 0.75])
ax6.set_yticks([0.00, 0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70])
ax6.set_yticklabels(['{:.2f}'.format(x) for x in [0.00, 0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70]], fontsize=8)
ax6.set_ylabel(r'Lateral FWHM ($\lambda_{\text{BSW}}$)', fontsize=8, color='r')
...
ax6.arrow(0.47, 0.22, -0.15, 0.01, linewidth=0.5,
...
ax5.scatter(feat, depth, c='k', marker='x', lw=0.6, s=10, zorder=2)

fwhm.mask[idx] = False
depth.mask[idx] = False
ax5.scatter(feat[idx[0]], depth[idx[0]], c='b', marker='o', lw=0.0, s=30, zorder=2)
ax5.scatter(feat[idx[1]], depth[idx[1]], c='b', marker='D', lw=0.0, s=20, zorder=2)
ax5.scatter(feat[idx[2]], depth[idx[2]], c='b', marker='^', lw=0.0, s=30, zorder=2)
ax5.scatter(feat[idx[3]], depth[idx[3]], c='b', marker='*', lw=0.0, s=40, zorder=2)
...
ax5.set_xticklabels([str(x) for x in [1.5, 1.25, 1.0, 0.75, 0.5, 0.25]], fontsize=8)
ax5.set_title(r'\textbf{(e)}', fontsize=12, loc='left')
ax5.set_xlabel(r'Feature size ($\lambda_{\text{BSW}}$)', fontsize=8)
...
plt.savefig('foc_depth.png', bbox_inches='tight', dpi=300)
